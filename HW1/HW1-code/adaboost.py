...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        for feature in features:
          weakClf = WeakClassifier(feature)
          total_error = 0
          for x, y, w in zip(iis, labels, weights):
            total_error += w * abs(weakClf.classify(x) - y)
          
          # Bonus code (Part 6)
          if total_error > 0.5:
            total_error = 1 - total_error
            weakClf = WeakClassifier(feature, polarity = -1)
          # End of Bonus code (Part 6)
